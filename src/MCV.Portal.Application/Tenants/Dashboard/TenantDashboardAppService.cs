...
namespace MCV.Portal.Tenants.Dashboard
{
    ...
    public class TenantDashboardAppService : PortalAppServiceBase, ITenantDashboardAppService
    {
        ...
        public TenantDashboardAppService (IRepository<Birthday> birthdayRepository)
        {
        ...
        public ListResultDto<MyBirthday> CheckMyBirthday()
        {
            var currentuser = GetCurrentUser().SAPCode;
            var me = _birthdayRepository.GetAll().ToList().Where(x => x.Id == currentuser);

            return new ListResultDto<MyBirthday>(ObjectMapper.Map<List<MyBirthday>>(me));
        }
